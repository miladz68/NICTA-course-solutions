...
noneof ::
  Chars
  -> Parser Char
noneof ls = satisfy (not.flip elem ls)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok ch ch2 = between (is ch) (is ch2)
...
hex ::
  Parser Char
hex = thisMany 4 (satisfy isHexDigit)
                 >>= \a -> case readHex a of
                                Empty -> failed
                                Full a' -> valueParser (toEnum a')
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby pa ps = sepby1 pa ps ||| list failed
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lb = satisfy (\pre -> and (lb <*> pure pre))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lb =satisfy (\pre -> or (lb <*> pure pre))
